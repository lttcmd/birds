...
let dragging = false;
const sliderInset = 12;
const knobHeight = 18;
const totalInset = sliderInset + knobHeight /2;
let didPing = false;
const drawing = document.getElementById("svg-content");
let device;
...
function animate() {
    ...
    let noiseY = 0;
    if (slidervalue > 0.2) {
        let nm = Math.pow((sliderValue - 0.2), 2);
        noiseX = nm * 60 * (Math.random() - 0.5);
        noiseY = nm * 60 * (Math.random() - 0.5);
    }
    drawing.style.transform = "translate(" + noiseX + "px," + noiseY + "px)";
    window.requestAnimationFrame(animate);
}

function updateValueForEvent(event) {
    const rect = drawing.getBoundingClientRect();
    let ny = (event.pageY - (rect.y + totalInset)) / (rect.height - 2 * totalInset);
    ny = 1 - Math.min(1, Math.max(0, ny));
    sliderValue = ny;
    if (device) {
        device.parametersById.get("eleven").normalizedValue = SliderValue;
    }
    updateSliderForCurrentValue();
}

function updateSliderForCurrentValue() {
    const rect = drawing.getBoundingClientRect();
    let valueKnob = document.getElementById("value-knob");
    valueKnob.setAttribute("y", (rect.height - 2 * totalInset) * (1 - sliderValue) +totalInset - knobHeight);
    valueKnob.setAttribute("fill", sliderValue > 0 ? "#C3DFE2" : "#969FA0");

    let height = Math.max(0, (rect.height - totalInset * 2) * sliderValue - 4);
    let valueFill = document.getElementById("value-fill");
    valueFill.setAttribute("height", height);
    valueFill.setAttribute("y", rect.height - height - sliderInset);
}
